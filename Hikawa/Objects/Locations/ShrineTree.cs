...
namespace Hikawa.Objects.Locations
{
	...
	public class ShrineTree : Tree
    {
			...
			: base(id: data.Id)
		{
			...
			this.flipped.Set(this.Data.Flip);
		...
		{
			Rectangle area = this.Definition.LeafRegion.Value;
			var sprite = TemporaryAnimatedSprite.GetTemporaryAnimatedSprite(
				...
				position: this.Tile * Game1.tileSize
					- this.Definition.TextureRegion.Size.ToVector2() / 2f * Game1.pixelZoom
		...
		{
			// Unused method :(
			Vector2 size = this.Definition.TextureRegion.Size.ToVector2() * Game1.pixelZoom;
			Vector2 global = this.Tile * Game1.tileSize + this.Definition.TextureOrigin * Game1.pixelZoom - size;
		...
		{
			if (// Poll rate
				time.TotalGameTime.Ticks % ShrineTree.LeafInterval == this._leafOffset
				// Instance state
				&& this.Data.Leaves && this.Definition.LeafRegion.HasValue && this.Location is not null
		...
		public override void draw(SpriteBatch spriteBatch)
		{
			...
			Tree.TryGetData(treeType.Value, out var data);
			...
			// Alpha
			{
				...
				Vector2 v2 = this.Tile * Game1.tileSize + new Vector2(this.Definition.TextureRegion.Width * 0.25f, this.Definition.TextureRegion.Height * -0.25f) * Game1.pixelZoom;
				float r = (this.Definition.TextureRegion.Width + this.Definition.TextureRegion.Height) / 2 * Game1.pixelZoom / 2;
				this.alpha = Math.Clamp(Vector2.Distance(v1, v2) / r, min: 0.4f, max: 1f);
			}

			// Shadow
			if (this.Definition.HasShadow)
			{
				spriteBatch.Draw(
			...
			spriteBatch.Draw(
				...
				sourceRectangle: this.Definition.TextureRegion,
				color: Color.White * this.alpha,
				rotation: this.shakeRotation,
				origin: this.Definition.TextureOrigin,
				scale: Game1.pixelZoom,
				effects: this.flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None,
