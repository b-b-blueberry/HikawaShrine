...
namespace Hikawa.Objects.Menus
{
    public class EmaMenu : IClickableMenu
	{
		...
		public EmaMenu(State state, bool addRootToStack)
		{
			...
			_texture = Helper.ModContent.Load<Texture2D>(AssetManager.ExtraSpritesAssetName);
		...
		{
			var hsl = Utils.ColorConverter.RGBtoHSL(color);
			if (hsl == Vector3.Zero)
				return;
			...
			color = Utils.ColorConverter.HSLtoRGB(hsl, color);
		}
		...
		*/
		...
		public override void update(GameTime time)
		{
			...
			else if (Game1.season != Season.Winter && Game1.random.NextDouble() < animChance)
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			var state = _stack.Peek();
			var view = Game1.graphics.GraphicsDevice.Viewport.TitleSafeArea;
			...
			if (state == State.Root)
				b.Draw(
					Game1.fadeToBlackRect,
					view,
					Color.Black * skyBlackoutMaxOpacity * Utils.GetProgressFromEveningIntoNighttime());
			*/
			// Midground - Ema frame
			...
			if (true)
				b.Draw(
					Game1.fadeToBlackRect,
					view,
					SkyDarkColor * Utils.GetProgressFromEveningIntoNighttime() * skyBlackoutMaxOpacity);
			*/
			// Debug text
			if (false)
