...
namespace Hikawa.Modules
{
    public static class MiniSit
    {
        ...
        private static void PlayerOnWarped(object sender, WarpedEventArgs e)
        {
        ...
        private static void InputOnButtonPressed(object sender, ButtonPressedEventArgs e)
        {
            ...
            if (!IsPlayerSittingDown)
            {
                ...
                var property = Game1.currentLocation.doesTileHaveProperty(
                    xTile: (int)position.X,
                    yTile: (int)position.Y,
                    propertyName: ActionName,
                    layerName: "Buildings")
                    is string s ? s.Split(' ') : null;
        ...
        {
            return IsPlayerSittingDown || Game1.player == null || Game1.player.isRidingHorse();
        }
        ...
        private static void SitDownStart(Vector2 position, int direction)
        {
            ...
            _playerLastStandingLocation = Game1.player.Tile;
        ...
        private static void SitDownEnd()
        {
            ...
            if (Game1.season != Season.Winter || !_playerLastSittingProperties.Any(p => p == "butt"))
                return;

            var position = new Vector2(
                Game1.player.lastPosition.X,
                Game1.player.lastPosition.Y - 32 - 64);
            var id = ModConsts.GenericID
            ...
            var layer = Game1.player.getStandingPosition().Y / 10000f - 1f / 1000f;
